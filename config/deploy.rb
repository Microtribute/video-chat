...
role :web,    "matrix.itima.ru"
role :app,    "matrix.itima.ru"
role :db,     "matrix.itima.ru", :primary => true
...
set :default_environment, {
		'LANG' => 'en_US.UTF-8',
		'PATH'         => "/usr/local/rvm/gems/ruby-1.9.2-p290/bin:/usr/local/rvm//bin:/usr/local/rvm/ruby-1.9.2-p290/bin:$PATH",
...
namespace :deploy do
	...
	task :finalize_update, :except => { :no_release => true } do
      ...
      ln -sf #{shared_path}/config.yml #{latest_release}/config/config.yml &&
      ln -sf #{shared_path}/facebook.yml #{latest_release}/config/facebook.yml
